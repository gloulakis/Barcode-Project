...
public class MetaDataRegressionTest extends BaseTestCase {
			...
			while (this.rs.next()) {
				...
				if ((typeName.indexOf("BINARY") == -1)
					...
					if (!alreadyDoneTypes.containsKey(typeName)) {
						...
						types.append(safeTypeName.toString() /* for JDK1.3.1 */);
						types.append("Column ");
	...
	public void testBug7033() throws Exception {
		...
		try {
			...
			assertEquals(testString, retrString);
	...
	public void testBug11781() throws Exception {
	     ...
	     assertEquals("comment; APPFK(`C1`) REFER `" + catalog + "`/ `app tab` (`C1`)", rs.getString(3));
	     
	     rs.close();
	     
	     rs = this.conn.getMetaData().getImportedKeys(this.conn.getCatalog(), null, "app tab");
	     
	     assertTrue(this.rs.next());
	     
	     rs = this.conn.getMetaData().getExportedKeys(this.conn.getCatalog(), null, "app tab");
	...
	private void checkRsmdForBug13277(ResultSetMetaData rsmd) throws SQLException {
		...
		if (versionMeetsMinimum(4, 1)) {
			...
			assertEquals(true, rsmd.isDefinitelyWritable(1));
			assertEquals(false, rsmd.isReadOnly(1));
			assertEquals(true, rsmd.isWritable(1));
		}
	}
