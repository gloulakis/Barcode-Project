...
public class StatementRegressionTest extends BaseTestCase {
	...
	/**
	 * Tests fix for BUG#15024 - Driver incorrectly closes streams passed as arguments
	 * to PreparedStatements.
	 * 
	...
	public void testBinaryType() throws Exception {
		if (versionMeetsMinimum(4, 1, 0)) {
			...
			createTable(
					"t1",
					"(a varchar(127) NOT NULL default '.', b binary(255), PRIMARY KEY (a)) Engine InnoDB CHARACTER SET utf8");

			PreparedStatement pstmt1 = conn
					.prepareStatement("INSERT INTO t1 VALUES(?, ?)");
			PreparedStatement pstmt2 = conn
					.prepareStatement("SELECT b FROM t1 WHERE a = ?");

			pstmt1.setString(1, "MAXVALUES");
			pstmt1.setBytes(2, origBytes);
			pstmt1.execute();

			pstmt2.setString(1, "MAXVALUES");
			...
			fetchedBytes = rs.getBytes(1);
			

			String origBytesAsHex = getByteArrayString(origBytes);
			String fetchedBytesAsHex = getByteArrayString(fetchedBytes);

			assertEquals(origBytesAsHex, fetchedBytesAsHex);
			
			assertEquals("[B", this.rs.getObject(1).getClass().getName());
			assertEquals(this.versionMeetsMinimum(5, 0, 0) ? Types.BINARY : Types.VARBINARY, this.rs.getMetaData().getColumnType(1));
			assertEquals("[B", this.rs.getMetaData().getColumnClassName(1));
		}
	}

	/** 
	 * Tests fix for BUG#13255 - Reconnect during middle 
	 * of executeBatch() should not happen.
	 * 
	 * @throws Exception if the test fails.
	 */
